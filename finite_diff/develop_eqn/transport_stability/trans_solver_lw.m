function u = trans_solver_lw(Nt,Nx,lambda,a,u0)
% TRANS_SOLVER_LW solves transport equation $u_t + a u_x = 0$ 
...
ind = ones(1, Nx);
A1 = (1-(a*lambda)^2).*diag(ind);
A2 = -0.5*(a*lambda)*(1-a*lambda).*diag(ind(2:Nx),1);
A3 = 0.5*(a*lambda)*(1+a*lambda).*diag(ind(2:Nx),-1);
A = A1 + A2 + A3;
